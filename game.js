...
/*
  Neon Beat Duel
  Original HTML/CSS/vanilla JS rhythm game.
  Edit DEMO_CHART to change the song chart. Notes use milliseconds from song start.
*/
...
const TONE_BY_DIRECTION = { left: 261.63, down: 329.63, up: 392, right: 523.25 };

const HIT_WINDOWS = { sick: 45, good: 90, bad: 135, miss: 170 };
const JUDGMENT_DATA = {
  sick: { label: "SICK!!", score: 350, accuracy: 1, health: 3.2 },
  good: { label: "GOOD!", score: 220, accuracy: 0.78, health: 2 },
  bad: { label: "BAD", score: 80, accuracy: 0.42, health: 0.8 },
  miss: { label: "MISS", score: 0, accuracy: 0, health: -6 }
};
...
const lanes = Object.fromEntries(
  DIRECTIONS.map((direction) => [direction, document.querySelector(`.lane[data-direction="${direction}"]`)])
);
...
const deviceState = { isMobile: false, hasTouch: false };
...
function updateViewportSize() {
  ...
  const tooShortForGameplay = deviceState.isMobile && currentScreen === "gameplay" && window.innerHeight < 390;
  if (elements.orientationWarning) elements.orientationWarning.classList.toggle("hidden", !tooShortForGameplay);
}

function detectDeviceSupport() {
  const coarsePointer = window.matchMedia("(pointer: coarse)").matches;
  const hasTouch = navigator.maxTouchPoints > 0 || "ontouchstart" in window || coarsePointer;
...
function playBeatSound(beatIndex) {
  if (beatIndex % 4 === 0) playTone(82, 0.09, "sine", 0.22);
  else if (beatIndex % 4 === 2) playTone(170, 0.055, "triangle", 0.12);
  else playTone(110, 0.035, "sine", 0.07);
}

function startSongAudio() {
  // Placeholder song uses generated Web Audio beeps in updateBeat().
}
...
function resetGameState() {
  ...
  notes = DEMO_CHART.map((chartNote, index) => {
    const lane = lanes[chartNote.direction];
    const noteElement = document.createElement("div");
    noteElement.className = `note note-${chartNote.direction}`;
    noteElement.textContent = ARROW_SYMBOLS[chartNote.direction];
    noteElement.dataset.index = String(index);
    lane.querySelector(".note-stack").appendChild(noteElement);
    return {
      id: index,
      time: chartNote.time,
      direction: chartNote.direction,
      element: noteElement,
      hit: false,
      missed: false
    };
...
function updateHud() {
  ...
  elements.accuracyText.textContent = `${accuracy.toFixed(1)}% ${getRank(accuracy)}`;
...
function updateNotePositions(songTime) {
  ...
  notes.forEach((note) => {
    ...
    note.element.style.opacity = y > -90 && y < laneHeight + 100 ? "1" : "0";
  });
}
...
function findBestNote(direction, songTime) {
  ...
  notes.forEach((note) => {
    ...
    if (Math.abs(delta) <= HIT_WINDOWS.bad && Math.abs(delta) < Math.abs(bestDelta)) {
      bestNote = note;
      bestDelta = delta;
    }
  });
  return { note: bestNote, delta: bestDelta };
}
...
function hitNote(note, judgment) {
  ...
  if (judgment === "sick") shakeStage("light");
  setTimeout(() => note.element.remove(), 190);
  updateHud();
}
...
function missNote(note) {
  ...
  applyMissPenalty();
  setTimeout(() => note.element.remove(), 250);
}
...
function togglePause() {
  ...
  if (paused) {
    ...
    songStartTime += performance.now() - pauseStartedAt;
    elements.pauseOverlay.classList.add("hidden");
...
function endGame(didWin) {
  ...
  elements.resultRank.textContent = getRank(accuracy);
  showScreen("results");
}
...
function flashInput(direction) {
  ...
  setTimeout(() => receptor.classList.remove("pressed"), 120);
  const touchButton = document.querySelector(`[data-touch="${direction}"]`);
  if (touchButton) {
    touchButton.classList.add("active");
    setTimeout(() => touchButton.classList.remove("active"), 120);
  }
}
...
function createSpark(direction) {
  ...
  lanes[direction].appendChild(spark);
...
function shakeStage(strength) {
  ...
  setTimeout(() => elements.stage.classList.remove(className), strength === "heavy" ? 180 : 130);
}
...
function animatePlayerHit() {
  ...
  setTimeout(() => elements.player.classList.remove("hit"), 170);
}
...
function bindEvents() {
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      if (currentScreen === "title" || currentScreen === "results") startCountdown();
      return;
    }
    if (event.key === "Escape") {
      if (currentScreen === "gameplay") togglePause();
      return;
    }
  ...
  $$(".touch-controls button").forEach((button) => {
    ...
    button.addEventListener("lostpointercapture", () => button.classList.remove("active"));
  });

  window.addEventListener("resize", detectDeviceSupport);
  window.addEventListener("orientationchange", () => setTimeout(detectDeviceSupport, 150));
  document.addEventListener("touchmove", (event) => {
    if (currentScreen === "gameplay") event.preventDefault();
  }, { passive: false });
...
function boot() {
  ...
  bindEvents();
  detectDeviceSupport();
  setTimeout(() => showScreen("title"), 850);
}
